...
const SelectBoard = ({markets, setMarkets, market, setMarket, boards, setBoards, board, setBoard, lyra}) => {
    ...
    const handleExpiryChange = async (event) => {
        let currentBoard = boards.find(board => board.expiryTimestamp === Number(event.target.value))
        ...
        <form>
            ...
            <select value={board.expiryTimestamp} onChange={handleExpiryChange}>
                ...
                {boards.map((board) => {
                    ...
                    return <option key={expiry} value={expiry}>{convertDate(expiry)}</option>
                })
